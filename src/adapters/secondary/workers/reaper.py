...
class ReaperRunner:
    ...
    def __init__(
        ...
        self._check_interval = (
            ...
            else settings.REAPER_CHECK_INTERVAL_SECONDS
        )
        self._min_idle_ms = (
            min_idle_seconds if min_idle_seconds is not None else settings.REAPER_MIN_IDLE_SECONDS
        ) * 1000
        self._consumer_name = f"reaper-{uuid4().hex[:8]}"

    async def run(self):
        logger.info(
            f"Reaper started (consumer={self._consumer_name}). Watching for tasks idle > {self._min_idle_ms}ms..."
        )
            ...
            try:
                ...
                if tasks:
                    ...
                    for stream_id, task in tasks:
                        ...
                        logger.info(
                            f"Resurrected task execution_id={task.execution_id} node_id={task.node_id}"
                        )
