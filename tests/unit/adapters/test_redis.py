...
async def test_redis_message_broker_acknowledge():
    ...
    mock_redis.xack.assert_called_with(RedisMessageBroker.TASK_STREAM, RedisMessageBroker.TASK_GROUP, "msg-1")
    
    await broker.acknowledge_completion("msg-2")
    mock_redis.xack.assert_called_with(RedisMessageBroker.COMPLETION_STREAM, RedisMessageBroker.COMPLETION_GROUP, "msg-2")
...
async def test_redis_message_broker_claim_stalled():
    ...
    mock_redis.xautoclaim.return_value = (
        ...
        [("msg-1", {
            "id": "t1", "execution_id": "e1", "node_id": "n1", 
            "handler": "h1", "config": json.dumps({"key": "val"})
        })]
    )
