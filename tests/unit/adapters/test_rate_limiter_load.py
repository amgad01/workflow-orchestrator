...
class TestRateLimiterLoadPerformance:
    ...
    async def test_concurrent_rate_limit_checks(self, rate_limiter, mock_redis):
        ...
        tasks = [
            rate_limiter.check_rate_limit("user1", limit=100, window_seconds=60) for _ in range(50)
        ]
...
class TestRateLimiterConcurrency:
    ...
    async def test_race_condition_handling(self):
        ...
        # Concurrent requests
        tasks = [
            rate_limiter.check_rate_limit("user", limit=5, window_seconds=60) for _ in range(10)
        ]
