...
# Cyclic DAG
res=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/workflow" \
...
for payload in "$PAYLOAD_DIR"/*.json; do
    ...
    # Submission
    submit_res=$(curl -s -X POST "$BASE_URL/workflow" -H "Content-Type: application/json" -d @"$payload")
    ...
    # Trigger
    curl -s -X POST "$BASE_URL/workflow/trigger/$exec_id" > /dev/null
    
    # Extended Polling for Complex DAGs (60s limit)
    state="PENDING"
    echo -ne "  ⏳ Progress: "
    for i in {1..60}; do
        status_res=$(curl -s "$BASE_URL/workflows/$exec_id")
    ...
    if [ "$state" == "COMPLETED" ]; then
        result_res=$(curl -s "$BASE_URL/workflows/$exec_id/results")
...
BURST_IDS=()
for i in {1..5}; do
    req=$(curl -s -X POST "$BASE_URL/workflow" -H "Content-Type: application/json" -d '{"name":"Burst","dag":{"nodes":[{"id":"n","handler":"input"}]}}')
    id=$(echo "$req" | grep -o '"execution_id":"[^"]*"' | cut -d'"' -f4)
    BURST_IDS+=($id)
    curl -s -X POST "$BASE_URL/workflow/trigger/$id" > /dev/null
done
...
for id in "${BURST_IDS[@]}"; do
    for j in {1..10}; do
        s=$(curl -s "$BASE_URL/workflows/$id" | grep -o '"status":"[^"]*"' | cut -d'"' -f4)
...
cancel_req=$(curl -s -X POST "$BASE_URL/workflow" -H "Content-Type: application/json" -d '{"name":"Cancel","dag":{"nodes":[{"id":"long","handler":"call_external_service"}]}}')
cancel_id=$(echo "$cancel_req" | grep -o '"execution_id":"[^"]*"' | cut -d'"' -f4)
curl -s -X POST "$BASE_URL/workflow/trigger/$cancel_id" > /dev/null
curl -s -X DELETE "$BASE_URL/workflow/$cancel_id" > /dev/null

echo -ne "  ⏳ Verifying CANCELLED status"
for i in {1..10}; do
    s=$(curl -s "$BASE_URL/workflows/$cancel_id" | grep -o '"status":"[^"]*"' | cut -d'"' -f4)
...
rate_res=$(curl -s -i -X POST "$BASE_URL/workflow" -H "Content-Type: application/json" -d '{"name":"RateCheck","dag":{"nodes":[{"id":"n1","handler":"input"}]}}')
...
fail_submit=$(curl -s -X POST "$BASE_URL/workflow" -H "Content-Type: application/json" -d "$fail_payload")
fail_exec_id=$(echo "$fail_submit" | grep -o '"execution_id":"[^"]*"' | cut -d'"' -f4)

if [ ! -z "$fail_exec_id" ]; then
    curl -s -X POST "$BASE_URL/workflow/trigger/$fail_exec_id" > /dev/null

    echo -ne "  ⏳ Waiting for max retries & DLQ entry"
    for i in {1..30}; do
        status_res=$(curl -s "$BASE_URL/workflows/$fail_exec_id")
        state=$(echo "$status_res" | grep -o '"status":"[^"]*"' | cut -d'"' -f4)
    ...
    # Verify DLQ entry
    dlq_res=$(curl -s "$BASE_URL/admin/dlq")
    ...
    if [ ! -z "$entry_id" ]; then
        retry_res=$(curl -s -X POST "$BASE_URL/admin/dlq/$entry_id/retry")
...
cb_submit=$(curl -s -X POST "$BASE_URL/workflow" -H "Content-Type: application/json" -d "$cb_payload")
cb_exec_id=$(echo "$cb_submit" | grep -o '"execution_id":"[^"]*"' | cut -d'"' -f4)
curl -s -X POST "$BASE_URL/workflow/trigger/$cb_exec_id" > /dev/null

sleep 2
cb_status=$(curl -s "$BASE_URL/workflows/$cb_exec_id")
